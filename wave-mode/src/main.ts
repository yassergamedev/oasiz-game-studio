...
class WaveModeGame {
  ...
  private updateRuntimePalette(): void {
    ...
    const phase = meters / Math.max(1, CONFIG.PALETTE_SHIFT_METERS) + tSec * CONFIG.PALETTE_TIME_SPEED;
  ...
  private drawWheels(wheels: Wheel[]): void {
    ...
    for (const w of wheels) {
      ...
      diskGrad.addColorStop(0.55, CONFIG.TRAIL);
      ...
      for (let i = 0; i < 10; i++) {
        ...
        ctx.fillStyle = i % 3 === 0 ? "rgba(255,255,255,0.9)" : CONFIG.TRAIL;
  ...
  private drawBlock(b: Block): void {
    ...
    neb.addColorStop(0.35, CONFIG.TRAIL);
